...
def create_mel_bank(fps, min_frequency, max_frequency, sampling_rate, number_of_samples, min_volume_threshold):
...
mel_y = None
mel_x = None
create_mel_bank(60, 200, 12000, 44000, 24, 1e-7)
...
class AudioProcessing():
    def __init__(self, fps, min_frequency, max_frequency, sampling_rate, number_of_samples, min_volume_threshold):
        ...
        self.y_roll = np.random.rand(
            4, self.samples_per_frame) / 1e16

        self.fft_plot_filter = ExpFilter(
            np.tile(1e-1, number_of_samples),
            alpha_decay = 0.5,
            alpha_rise = 0.99
        )
    ...
    def rfft(self, data, window = None):
        ...
        xs = np.fft.rfftfreq(len(data), 1.0 / sampling_rate)
        return xs, ys
    ...
    def fft(self, data, window = None):
        ...
        xs = np.fft.fftfreq(len(data), 1.0 / sampling_rate)
        return xs, ys
    ...
    def render(self, audio_samples):
        ...
        if vol < self.min_volume_threshold:
            ...
            return np.tile(0., self.number_of_samples)
        else:
