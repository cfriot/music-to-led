...
class PixelReshaper:
    ...
    def initActiveShape(self):
        ...
        self.strip_shape = self.strip_config.shapes[self.active_shape_index].shape
        self.strips = []
    ...
    def splitForStrips(self, strips, pixels):
        ...
        for i, strip_length in enumerate(self.strip_shape):
            if(self.strip_config.is_reverse and not self.strip_config.is_mirror):
                strips[i] = pixels[:, self.number_of_pixels - strip_length:]
            elif(self.strip_config.is_mirror and not self.strip_config.is_reverse):
            ...
            elif(self.strip_config.is_mirror and self.strip_config.is_reverse):
