...
class ModSwitcher:
    ...
    def changeMod(self):
        if(self.midi_datas):
            for self.midi_data in self.midi_datas:
                mode = self.midi_data["note"]
                if(mode == 0):
                    ...
                    print(self.strip_config.active_audio_channel_index, self.config.number_of_audio_ports)
                    ...
                    pixels = np.tile(.0, (3, self.serialToArduinoLedStrip.number_of_pixels))
                    self.serialToArduinoLedStrip.update(pixels)
                    self.visualizer.pixelReshaper.active_shape_index = self.strip_config.active_shape_index
                    self.visualizer.number_of_pixels = self.visualizer.pixelReshaper.number_of_pixels
                    self.visualizer.initVizualiser()
                    self.visualizer.resetFrame()
                    self.visualizer.pixelReshaper.initActiveShape()
                    self.serialToArduinoLedStrip.number_of_pixels = self.visualizer.pixelReshaper.number_of_pixels

                    print("Change shape to =>", self.visualizer.pixelReshaper.active_shape_index,
                          self.visualizer.pixelReshaper.strip_shape)
